...
const Login = () => {
  ...
  const [localBanReason, setLocalBanReason] = useState("");

  useEffect(() => {
    dispatch(reset());
    // Kiểm tra xem user có vừa bị ép văng ra không?
    const sessionBanMsg = sessionStorage.getItem("ban_msg");
    if (sessionBanMsg) {
      setLocalBanReason(sessionBanMsg);
      setShowBanModal(true); // show modal cấm
      sessionStorage.removeItem("ban_msg"); // xóa đi để F5 không bị lại
    }
  }, [dispatch]);

  useEffect(() => {
    // Fallback catch qua lỗi API 403
    if (isError && banReason) {
      setLocalBanReason(banReason);
