...
const UserAnalytics = ({ totalStudents, activeStudents }) => {
  ...
  const data = [
    { name: 'Đã tham gia khóa học', value: activeStudents },
    { name: 'Đã đăng ký', value: inactiveStudents },
  ];

  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col h-full text-left">
      <h3 className="text-lg font-bold text-gray-800 mb-1">Thống kê học viên</h3>
      ...
      <div className="flex-1 min-h-[200px] relative">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-8">
          ...
          <span className="text-xs text-gray-400 uppercase">Tổng</span>
        </div>
      </div>
    </div>
  );
