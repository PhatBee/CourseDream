...
export const sendReport = createAsyncThunk(
  "report/sendReport",
  async ({ type, targetId, reason, detail }) => {
    if (type === "course")
      return await reportService.reportCourse(targetId, reason, detail);
    if (type === "discussion")
      return await reportService.reportDiscussion(targetId, reason, detail);
    if (type === "reply")
      return await reportService.reportReply(targetId, reason, detail);
    ...
    builder
      ...
      .addCase(fetchReports.fulfilled, (state, action) => {
        ...
        // Nếu backend trả về { data: { reports: [...], pagination: {...} } }
          ...
          action.payload.data &&
          Array.isArray(action.payload.data.reports)
        ) {
