...
const ReviewFormMobile = ({ courseId, isEnrolled }) => {
  ...
  const handleSubmit = async () => {
    ...
    await dispatch(fetchReviews({ courseId })); // Gọi lại với chuẩn object arg
    setComment("");
    ...
    return (
      <View className="px-4 mb-4">
        ...
        <Text className="text-yellow-600">
          Bạn cần ghi danh khóa học để đánh giá.
        </Text>
      </View>
    );
  }
      ...
      <View className="flex-row items-center mb-2">
        {[1, 2, 3, 4, 5].map((num) => (
          <TouchableOpacity key={num} onPress={() => setRating(num)}>
            <Star
              size={22}
              color={num <= rating ? "#f59e0b" : "#e5e7eb"}
              fill={num <= rating ? "#f59e0b" : "transparent"}
            />
          </TouchableOpacity>
        ))}
